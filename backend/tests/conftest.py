...
def setup_test_database(request):
    ...
    # Do not drop tables if session includes E2E browser tests
    is_e2e = any("e2e" in str(getattr(item, "fspath", "")) for item in request.session.items)
    if not is_e2e:
...
def clean_database(request):
    # Do not wipe database during E2E browser tests
    if "e2e" in str(request.fspath) or "e2e" in request.node.nodeid:
        yield
        return
